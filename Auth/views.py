...
def json_download(request, filename):
    data = {object: 'AKL0000001', action: 'update-values', values: 'value'}
    json_str = serializers.serialize('json', data)
    response = HttpResponse(json_str, content_type='application/json')
    response['Content-Disposition'] = 'attachment; filename=edit.json'
    return response
